...
- 调用 `quartus_sim` 尝试运行 VWF 仿真，并把结果保存为 CVWF。
- 汇总 `.rpt`、`.summary`、`.log`、`.sof`、`.pof`、`.cvwf` 等结果文件。
## 效果演示
![codex.png](<assets/codex_vVVepQ.png>)
![VHDL.png](<assets/VHDL_s9J7lX.png>)
![SIMU.png](<assets/SIMU_1MR411.png>)
![pins.png](<assets/pins_x_x6tc.png>)
## 前置条件
...
| `run_vwf_simulation` | 运行 VWF 仿真，禁用 VWF 回写，并查找 `.cvwf` |
...
```
然后你就会得到一个带 VHDL、VWF 仿真波形的完整工程，而且引脚会自动配置好，
你只需要下载到开发板即可。
...
VWF 是输入激励文件，应该只保存 `clk`、`rst`、`enable` 这类输入信号的 `TRANSITION_LIST`。输出信号可以声明在 VWF 里用于显示，但不应该写输出波形激励。
...
```text
--simulation_results_format=CVWF --overwrite_waveform=off
```

这样可以避免 Quartus 把仿真输出回写进输入 `.vwf`，导致输入波形文件被输出结果污染。
